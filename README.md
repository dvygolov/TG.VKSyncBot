...
- При `VK_ENABLE_EDIT_SYNC=true`:
  - если `edited_channel_post` раньше не подходил под фильтр (например, без хештега), а после редактирования стал подходить, бот публикует его;
  - если у сообщения уже есть ровно один связанный VK-пост, бот пытается обновить его через `wall.edit`;
  - если у сообщения несколько VK-постов (например, из-за разбиения длинного текста), обновление пропускается.
